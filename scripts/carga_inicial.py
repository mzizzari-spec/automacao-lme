...
Funcao: Roda UMA VEZ SO para carregar os ultimos 3 meses de dados historicos
...
CABECALHO = [
    ...
    "Aluminio (US$/t)", "Var. Aluminio",
    "Dolar (R$/US$)", "Var. Dolar",
    "Cobre (R$/kg)", "Var. Cobre/kg",
    "Aluminio (R$/kg)", "Var. Aluminio/kg",
]

CABECALHO_CONSOLIDADO = CABECALHO + ["Mes"]
...
def eh_dia_util(d):
    ...
    if (d.day, d.month) in FERIADOS_FIXOS: return False
    if date(d.year, d.month, d.day) in feriados_moveis(d.year): return False
    return True

def dias_uteis_do_mes(ano, mes):
    _, ultimo = calendar.monthrange(ano, mes)
    return [date(ano, mes, d) for d in range(1, ultimo + 1) if eh_dia_util(date(ano, mes, d))]

def nome_aba(ano, mes):
    return "{}/{}".format(MESES_PT[mes-1], ano)

def conectar_google_sheets():
    creds_json = os.environ.get("GOOGLE_CREDENTIALS_JSON")
    if not creds_json: raise ValueError("Secret GOOGLE_CREDENTIALS_JSON nao encontrado!")
...
def buscar_dados_mes(ano, mes):
    ...
    print("\nBuscando dados de {}...".format(nome_aba(ano, mes)))
    ...
    for linha in tabela.find_all("tr"):
        ...
        if "media" in dia_texto.lower() or "media" in dia_texto.lower(): continue
        match = re.match(r"(\d+)/(\w+)", dia_texto)
        if not match: continue
        dia_num = int(match.group(1))
        mes_abrev_site = match.group(2)
        if mes_abrev_site != mes_nome: continue
        try:
        ...
        if cobre or aluminio:
            ...
            print("  {} -- Cobre: {} | Aluminio: {} | Dolar: {}".format(
                d.strftime("%d/%m/%Y"), cobre, aluminio, dolar))
    return dados_mes
...
def processar_mes(planilha, ano, mes, dados_reais):
    ...
    dias_nomes = ["Segunda", "Terca", "Quarta", "Quinta", "Sexta"]

    # Busca ultimo valor do mes anterior
    ...
    try:
        ...
        for linha in reversed(dados_ant):
            if len(linha) > 2 and linha[2] == "Real" and para_float(linha[3]):
                ultimo_ant = {"cobre": para_float(linha[3]), "aluminio": para_float(linha[5]), "dolar": para_float(linha[7])}
                break
    except: pass

    # Ultimo valor real disponivel para projecao
    ultimo_real = None
    ultimo_dolar_conhecido = ultimo_ant["dolar"] if ultimo_ant else None
    for d in sorted(dados_reais.keys(), reverse=True):
        if dados_reais[d]["dolar"] is not None:
    ...
    for d in dias_uteis:
        ...
        elif d < hoje and d not in dados_reais:
            # Dia passado sem dados - linha vazia para preenchimento manual
            linha_vazia = [data_str, dias_nomes[d.weekday()], "Real",
                          "", "", "", "", "", "", "", "", "", ""]
            todas_linhas.append(linha_vazia)
            semana_atual.append(linha_vazia)
            proximo = next((dd for dd in dias_uteis if dd > d), None)
            fim_semana = (d.weekday() == 4 or proximo is None or
                         proximo.isocalendar()[1] != d.isocalendar()[1])
            if fim_semana and semana_atual:
                todas_linhas.append(["Media Semana","","","","","","","","","","","",""])
                semana_atual = []
            continue
        elif ultimo_real and d >= hoje:
            tipo = "Projetado"
            cobre = ultimo_real["cobre"]; aluminio = ultimo_real["aluminio"]
            dolar = ultimo_real["dolar"] or ultimo_dolar_conhecido
        else:
            continue
        ...
        linha = [data_str, dias_nomes[d.weekday()],
        ...
        if fim_semana and semana_atual:
            ...
            media_ant = next((l for l in reversed(todas_linhas) if l and l[0] == "Media Semana"), None)
            ...
            todas_linhas.append(["Media Semana","","",m3,var3,m5,var5,m7,var7,m9,var9,m11,var11])
            semana_atual = []

        valor_anterior = {"cobre": cobre, "aluminio": aluminio, "dolar": dolar}

    reais = [l for l in todas_linhas[1:] if len(l) > 2 and isinstance(l[2], str) and l[2] == "Real"]
    todos = [l for l in todas_linhas[1:] if len(l) > 2 and isinstance(l[2], str) and l[2] in ("Real","Projetado")]
    ...
    todas_linhas.append([])
    todas_linhas.append(["Media Real","","",med_mes(reais,3),med_mes(reais,4),med_mes(reais,5),med_mes(reais,6),med_mes(reais,7),med_mes(reais,8),med_mes(reais,9),med_mes(reais,10),med_mes(reais,11),med_mes(reais,12)])
    todas_linhas.append(["Media Projetada","","",med_mes(todos,3),med_mes(todos,4),med_mes(todos,5),med_mes(todos,6),med_mes(todos,7),med_mes(todos,8),med_mes(todos,9),med_mes(todos,10),med_mes(todos,11),med_mes(todos,12)])

    aba.update(values=todas_linhas, range_name="A1")
    linhas_validas = [l for l in todas_linhas[1:] if len(l) > 2 and isinstance(l[2], str) and l[2] in ("Real","Projetado")]
    print("  '{}': {} reais + {} projetados".format(nome, len(reais), len(todos)-len(reais)))
    return linhas_validas, nome
...
def atualizar_consolidado(planilha, todos_os_dados):
    ...
    print("\nConsolidado: {} linhas.".format(len(linhas)-1))

def main():
    print("="*55)
    print("CARGA INICIAL -- Ultimos 3 meses")
    print("="*55)
